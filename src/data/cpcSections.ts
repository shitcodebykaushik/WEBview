...
export const cpcData: CPCData[] =[
  ...
  export const transformCPCData = (data: CPCData[]): LegalSection[] => {
   
    const chaptersMap = new Map<number, LegalSection>();
    
    data.forEach(item => {
      const sectionNumber = typeof item.section === 'string' ? parseInt(item.section, 10) : item.section;
      const chapterNumber = Math.floor((sectionNumber - 1) / 10) + 1;
      ...
      const chapter = chaptersMap.get(chapterNumber)!;
      chapter.sections.push({
        number: item.section.toString(),
  ...
  function getChapterTitle(chapterNumber: number): string {
    const titles: Record<number, string> = {
        ...
        11: "Execution of Decrees and Orders",
        12: "Suits in Particular Cases",
        13: "Suits by or Against Government and Public Officers",
        14: "Suits Relating to Mortgages of Immovable Property",
        15: "Appeals",
      };
      
    
    return titles[chapterNumber] || "Miscellaneous";
  }